...
class Admin_model extends CI_Model
{
   ...
   public function get_mock_detail($id)
    ...
    public function add_mock_title($upload_data = '')
    {
        ...
        $info['course_id'] = $this->input->post('course_id') ? $this->input->post('course_id', TRUE) : NULL;
    ...
    public function update_subcategory()
    {
        ...
        switch ($name) {
            case 'sub_cat_name':
                ...
                break;
            case 'cat_id':
                $data['cat_id'] = $this->input->post('value', TRUE);
                break;
            default:
                return FALSE;
                break;
    ...
    public function update_mock($id)
    {
        date_default_timezone_set($this->session->userdata['time_zone']);
        $info = array();
        $info['category_id'] = $this->input->post('category', TRUE);
        ...
        $info['exam_created'] = date('Y-m-d H:i:s');
        $info['public'] = $this->input->post('public', TRUE);
        $info['course_id'] = $this->input->post('course_id', TRUE)?:NULL;
        $info['last_modified_by'] = $this->session->userdata['user_id'];
    ...
    {
        $questions = $this->db->select('ques_id')->where('exam_id', $id)->get('questions')->result();

        foreach ($questions as $question) {
            $this->delete_question_with_answers($question->ques_id);
        }
     ...
     * @return boolean
     */
    public function delete_question_with_answers($ques_id)
    {
        $this->db->where('ques_id', $ques_id);
