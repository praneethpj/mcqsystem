...
class System_control extends MS_Controller 
{
    ...
    public function index($code = '', $message = '')
    {
        $message = '<div class="alert alert-success alert-dismissable">'
                ...
                . 'System Info updated successfully! Please login using this info and change the info after login.'
                . 'Role: "Super Admin" | Email: "superadmin@demo.com" | Password: "123456"'
                . '</div>';
    ...
    {
        if (!$this->session->userdata('log') || $this->session->userdata('user_role_id') > 2){
            $message = '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="TRUE">&times;</button>Login as Admin to view this page.</div>';
            $this->session->set_flashdata('message', $message);
            redirect(base_url());
        }
