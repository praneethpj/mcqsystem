...
$(document).ready(function() {
   //alert("aa");
      ...
      var questioncategory=  $("#questioncategory").find('option:selected').attr('id');
      var subject_id=  $("#subject_id").find('option:selected').attr('id');
      var term=  $("#term").find('option:selected').attr('id');
      //var complexiety=  $("#complexiety").find('option:selected').attr('id');
      var medium=  $("#medium").find('option:selected').attr('id');
 
 //alert(term);

  $.ajax( {
      url: baseUrl+'/index.php/admin_control/add_question_basic_data/',
      type: 'POST',
      data: { 
        'category' : questioncategory, 'medium' : medium, 'subject' : subject_id, 'term' : term
         },
...
<div class="block" id="content">
   ...
   <div   class="form-horizontal" method="post" role="form" name="questions"  id="questions">
    <div class="row">
    <div class="col-sm-12">
        <div class="row">
...
<option id= "<?php  echo $subjects->subject_id;?>"><?php echo  $subjects->subject_title ?> </option>
<?php } ?>
</select>
            ...
            <div class="row">
                <div class="col-xs-offset-1 col-xs-11 col-sm-offset-2 col-md-8">
                    ...
                    <button type="button" onclick="$('#progressBar').show();"  class="btn btn-info col-xs-offset-1" name="done" id="nextdata" value="done"><i class="glyphicon glyphicon-saved"></i>Next</button>
                </div>
            </div><br/>
            </div>
        </div>
