        ...
        <div class="row">
            <p class="text-muted">Noticeboard 
                <?php if ($this->session->userdata['user_role_id'] < 3) { ?>
                    <a class="btn custom_navbar-btn btn-primary pull-right col-sm-3" href="<?php echo base_url('index.php/noticeboard/add'); ?>"><i class="glyphicon glyphicon-plus-sign"></i>&nbsp; Add Notice </a>
                <?php } ?>
            </p>
        </div>
    </div>
                                ...
                                <tr class="<?= ($i & 1) ? 'even' : 'odd'; ?>">
                                    ...
                                    <td class="col-xs-3 text-center">
                                        <div class="btn-group">
                                            ...
                                            <a class="btn btn-default btn-sm" href = "<?php echo base_url('index.php/noticeboard/edit/' . $notice->notice_id); ?>"><i class="glyphicon glyphicon-edit"></i><span class="invisible-on-md">  Modify</span></a>
                                            <?php if ($this->session->userdata['user_role_id'] <= 2) { ?>
                                                <a onclick="return delete_confirmation()" href = "<?php echo base_url('index.php/noticeboard/delete/' . $notice->notice_id); ?>" class="btn btn-sm btn-default"><i class="glyphicon glyphicon-trash"></i><span class="invisible-on-md">  Delete</span></a>
                                                    <?php } ?>
                                        </div>
                                    </td>
                                </tr>
